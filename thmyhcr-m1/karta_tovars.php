...
<body>
    ...
    <div class="tovar-container">
        ...
        $result = mysqli_query($link, "SELECT * FROM tovars, categories WHERE tovars.id_cat = categories.id_cat;");
        ...
        ?>
            <div class="tovar">
                ...
                <h3 class="tovar__name"><?= htmlspecialchars($row['name']) ?></h3><ul>
                    <li><span>Цена:</span> <?= htmlspecialchars($row['cena']) ?></li>
                    <li><span>Количество:</span> <?= htmlspecialchars($row['kol']) ?></li>
                    <li><span>Срок годности:</span> <?= htmlspecialchars($row['srok']) ?></li>
                    <li><span>Категория:</span> <?= htmlspecialchars($row['category']) ?></li>
                </ul>

                   <a href="podrobnee.php?id=<?= urlencode($row['id'] ?? 1) ?>" class="tovar__btn">
                 Подробнее</a>
            </div>
        <?php 
        } 
